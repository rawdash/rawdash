...
6. **Add the connector's brand icon** at `packages/connectors/<id>/icon.svg`. This is a required, committed asset (the docs generator fails if it's missing). **Every connector's icon comes from the same source — [Simple Icons](https://simpleicons.org/) (CC0) — so the catalog stays visually consistent. Do NOT hand-draw, trace, or invent an icon.**
   - Take the glyph verbatim from `https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/<slug>.svg` (the `<slug>` is the lowercased vendor name, e.g. `intercom`, `stripe`).
   - Look up the vendor's Simple Icons brand hex in `https://raw.githubusercontent.com/simple-icons/simple-icons/develop/data/simple-icons.json` (match on `title`) and use that exact hex for **both** the SVG `fill` and `doc.brandColor`.
   - The committed file must match the sibling connectors' shape exactly: a single `<path>` with `role="img"`, a `<title>`, and `fill="#RRGGBB"` — and nothing else (no background `<rect>`, no `width`/`height`). Diff against an existing `icon.svg` (e.g. `packages/connectors/stripe/icon.svg`) before committing.
...
- Add `packages/connectors/<id>/icon.svg` (required, committed; the generator fails without it). Source it from [Simple Icons](https://simpleicons.org/) (the single source all connectors share — AWS services excepted, see step 6), match the sibling shape exactly (single `<path>`, `role="img"`, `<title>`, `fill="#RRGGBB"`, no `<rect>`/`width`/`height`), and set both the SVG `fill` and `doc.brandColor` to the vendor's Simple Icons brand hex.
- Add `packages/connectors/<id>/src/example.config.ts` (a type-checked `defineConfig`; the generator inlines it).
- Run `pnpm docs:connectors` and commit the generated `README.md`. The website docs page (`<id>.mdx`), public icon (`<id>.svg`), and catalog data (`apps/website/src/generated/connectors.ts`) are gitignored and regenerated at build time, so don't commit them. `pnpm docs:connectors:check` must pass (it's the CI drift guard for the committed READMEs).
- **If this connector had a placeholder/"planned" entry, remove it** from `scripts/connector-placeholders.ts` (match by `id` or vendor name). The same generator renders both real connectors and placeholders, so a graduated connector left in that list would show up twice. On merge to `main`, the `close-connector-discussions` workflow detects the removed `id` and closes its "Connector Requests" GitHub Discussion as resolved; if that workflow is ever skipped, close the discussion by hand.
