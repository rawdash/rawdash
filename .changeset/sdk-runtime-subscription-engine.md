---
'@rawdash/sdk-runtime': minor
'@rawdash/sdk-nextjs': minor
'@rawdash/core': minor
---

**New:** `@rawdash/sdk-runtime` — framework-agnostic auto-polling subscription engine that drives client-side dashboards from the schedule the server already publishes (`cachedAt` + `syncIntervalSeconds` on each `CachedWidget`).
