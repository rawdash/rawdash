...
Adds two producer-side enforcement helpers: `metricSample(resources, name, { ts, value, attributes })` types `attributes` to the named resource's declared dimensions/measures so an undeclared key (or a mirrored value) fails to typecheck; and `withMetricResourceGuard(handle, resources)` wraps a storage handle to drop non-finite-`value` samples and strip undeclared attribute keys at runtime (applied by the server/MCP sync paths).
