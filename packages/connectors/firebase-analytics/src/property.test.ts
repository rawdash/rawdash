...
const CONNECTOR_ID = 'firebase-analytics';
...
const docShapeExtra = (
  ...
  connectorResourceShapeViolations(
    FirebaseAnalyticsConnector.resources,
    storage,
    connectorId,
  );
...
function makeConnector(): FirebaseAnalyticsConnector {
  return new FirebaseAnalyticsConnector(
    { propertyId: '123456789', firebaseAppId: '1:1234567890:web:abcdef' },
    {
...
type DauWauMauSample = z.infer<
  typeof FirebaseAnalyticsConnector.schemas.dau_wau_mau
>;

describe('FirebaseAnalyticsConnector property tests', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('dau_wau_mau: sync upholds universal invariants for any valid API payload', async () => {
    const extra = (
      storage: InMemoryStorage,
      _connectorId: string,
      sample: DauWauMauSample,
    ): InvariantViolation[] => {
      const violations: InvariantViolation[] = [];
      const rows = sample.rows ?? [];
      const samples = metricStoreFor(storage).filter(
        (m) => m.name === 'firebase_dau_wau_mau',
      );
      if (samples.length !== rows.length) {
        violations.push({
          invariant: 'one firebase_dau_wau_mau metric per row',
          location: 'dau_wau_mau phase',
    ...
    await runPropertySyncTest({
      connectorClass: FirebaseAnalyticsConnector,
      resource: 'dau_wau_mau',
        ...
        rows: [
          {
            dimensionValues: dimensions.map((d) => ({
              value:
                d.name === 'date' || d.name === 'firstSessionDate'
                  ? '20250101'
                  : 'sample',
            })),
