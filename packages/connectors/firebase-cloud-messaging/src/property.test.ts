...
const docShapeExtra = (
  ...
  connectorResourceShapeViolations(
    FirebaseCloudMessagingConnector.resources,
    storage,
    connectorId,
  );

function makeConnector(): FirebaseCloudMessagingConnector {
...
describe('FirebaseCloudMessagingConnector property tests', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('messages_per_day: sync upholds universal invariants for any valid payload', async () => {
    await runPropertySyncTest({
      connectorClass: FirebaseCloudMessagingConnector,
  ...
  it('messages_per_topic: sync upholds universal invariants for any valid payload', async () => {
    await runPropertySyncTest({
      connectorClass: FirebaseCloudMessagingConnector,
      resource: 'messages_per_topic',
      connectorId: CONNECTOR_ID,
      runs: 50,
      extraInvariants: [docShapeExtra],
      run: async (sample, storage) => {
        installMock(sample);
        await makeConnector().sync(
          { mode: 'full', resources: new Set(['messages_per_topic']) },
          storage.getStorageHandle(CONNECTOR_ID),
        );
      },
    });
