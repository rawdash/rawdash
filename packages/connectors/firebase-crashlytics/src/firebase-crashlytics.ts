  ...
  top_issues: {
    ...
    fields: [
      ...
      },
      {
        name: 'title',
        description:
          'Blamed stack-frame symbol for the issue (the crashing function/method), taken from the most recent event for this issue id within the window.',
      },
      {
        name: 'subtitle',
        description:
          'Blamed stack-frame source file for the issue, taken from the most recent event for this issue id within the window.',
      },
      {
        name: 'app_id',
        description:
...
export function buildCrashesPerDaySql(args: {
  ...
  return [
    ...
    "    LOWER(IFNULL(platform, 'unknown')) AS platform,",
    '    application.display_version AS version,',
    "    NULLIF(user.id, '') AS user_id,",
    '    event_id,',
    '    error_type',
    `  FROM ${table}`,
...
export function buildTopIssuesSql(args: {
  ...
  return [
    ...
    "  LOWER(IFNULL(ANY_VALUE(platform HAVING MAX event_timestamp), 'unknown')) AS platform,",
    '  COUNT(DISTINCT event_id) AS event_count,',
    "  COUNT(DISTINCT NULLIF(user.id, '')) AS user_count,",
