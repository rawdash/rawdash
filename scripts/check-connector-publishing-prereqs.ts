...
function bootstrapBlurb(pkg: WorkspacePackage): string {
  ...
  return [
    ...
    ``,
    `    # ── 1. Pre-flight checks ─────────────────────────────────────────────`,
    `    # npm publish returns a confusing 404 on auth/permission failures and`,
    `    # npm trust is a no-op on old CLIs, so verify both first:`,
    `    npm --version                      # must be ≥ 11.10.0`,
    `    npm whoami                         # must print a maintainer with @rawdash publish rights`,
    ``,
    `    # ── 2. Upgrade npm if needed ─────────────────────────────────────────`,
    ...
    `    # If \`npm whoami\` printed an error like "ENEEDAUTH" or "not logged in",`,
    `    # authenticate now:`,
    `    npm login                          # opens a browser for the OAuth flow`,
    `    #`,
    `    # If \`npm whoami\` printed the wrong account, log out first and then`,
    `    # log back in:`,
    `    npm logout`,
    `    npm login`,
    `    #`,
    `    # Re-run \`npm whoami\` to confirm before continuing.`,
    ``,
    `    # ── 4. Bootstrap the package ─────────────────────────────────────────`,
    `    # Run from the root of your local checkout (worktrees work too — use`,
    `    # the worktree root, not the main repo). The path below is relative to`,
    `    # the repo root:`,
  ...
  recordError(
    [
      ...
      'Bootstrap each one from a maintainer machine with 2FA before merging. ' +
        'Each section below walks through pre-flight, upgrade, login, and the ' +
        'actual publish. The cd path is relative to your local repo root — ' +
        'use your worktree root if the package only exists on this branch:',
      '',
